...
use crate::parse::{
    decode_bytes, extract_sections, parse_version, split_notes_fields,
    ParsedChartEntry, ParsedSimfileData,
};
use crate::timing::{
    compute_timing_segments_cleaned, format_bpm_segments_like_itg, round_sig_figs_itg,
    roundtrip_bpm_itg, steps_timing_allowed, TimingData, TimingFormat, ROWS_PER_BEAT,
};
...
fn has_control(s: &str) -> bool {
    s.bytes().any(|b| b < 0x20 && !matches!(b, b'\t' | b'\n' | b'\r'))
}
...
pub(crate) fn parse_beat_or_row(raw: &str) -> Option<f64> {
    let mut s = raw.trim();
    let is_row = s.strip_suffix(['r', 'R']).map(|r| { s = r.trim_end(); true }).unwrap_or(false);
    let v = s.parse::<f32>().ok().filter(|v| v.is_finite())?;
    Some(if is_row { v as f64 / ROWS_PER_BEAT as f64 } else { v as f64 })
}
    ...
    for entry in param.split(',') {
        ...
        if let Some((b, v)) = t.split_once('=') {
            if let (Some(b), Some(v)) = (normalize_decimal(b), normalize_decimal(v)) {
                ...
                out.push_str(&b); out.push('='); out.push_str(&v);
            }
        }
    }
    out
...
pub fn clean_timing_map_cow(param: &str) -> Cow<'_, str> {
    if param.is_empty() { return Cow::Borrowed(""); }
    let dirty = param.split(',').any(|e| e.is_empty() || e.trim() != e || has_control(e));
    if dirty { Cow::Owned(clean_timing_map(param)) } else { Cow::Borrowed(param) }
}

// Unified tag processing - replaces 6 functions
#[inline]
fn map_tag<F: FnOnce(&str) -> String>(tag: Option<&[u8]>, f: F) -> String {
    tag.and_then(|b| std::str::from_utf8(b).ok()).map(f).unwrap_or_default()
}

#[inline]
fn map_tag_opt<F: FnOnce(&str) -> String>(tag: Option<&[u8]>, f: F) -> Option<String> {
    tag.and_then(|b| std::str::from_utf8(b).ok()).map(f).filter(|s| !s.is_empty())
}
...
fn decode_display_bpm_tag(tag: Option<&[u8]>) -> Option<String> {
    tag.map(decode_bytes).map(|s| s.trim().to_string()).filter(|s| !s.is_empty())
}
...
fn parse_float_prefix(s: &str) -> Option<f64> {
    ...
    let mut i = if b.first().map_or(false, |&c| c == b'+' || c == b'-') { 1 } else { 0 };
    let start = i;
    while i < b.len() && b[i].is_ascii_digit() { i += 1; }
    if i < b.len() && b[i] == b'.' { i += 1; while i < b.len() && b[i].is_ascii_digit() { i += 1; } }
    if i == start || (i == start + 1 && !b[start].is_ascii_digit()) { return None; }
    if i < b.len() && matches!(b[i], b'e' | b'E') {
        let e = i; i += 1;
        if i < b.len() && matches!(b[i], b'+' | b'-') { i += 1; }
        let ed = i;
        while i < b.len() && b[i].is_ascii_digit() { i += 1; }
        if ed == i { i = e; }
    }
    std::str::from_utf8(&b[..i]).ok()?.parse().ok().map(|v: f64| if v.is_finite() { v } else { 0.0 })
}
...
fn parse_display_bpm(tag: &str) -> Option<(f64, f64)> {
    ...
    if min_s.is_empty() { return None; }
    let min = (parse_float_prefix(min_s).unwrap_or(0.0) as f32) as f64;
    let max = max_s.filter(|s| !s.is_empty())
...
pub(crate) fn resolve_display_bpm(
    chart_tag: Option<&str>, actual_min: f64, actual_max: f64, rate: f64,
) -> (f64, f64, String) {
    let (mut min, mut max) = chart_tag
        .map(str::trim).filter(|s| !s.is_empty())
        .and_then(parse_display_bpm)
        .unwrap_or((actual_min, actual_max));
    if min <= 0.0 || max <= 0.0 { min = actual_min; max = actual_max; }
    let (smin, smax) = (min * rate, max * rate);
    let fmt = |v: f64| if rate == 1.0 { format!("{:.0}", v) } else {
        let s = format!("{:.1}", v);
        s.strip_suffix(".0").map_or(s.clone(), |t| t.to_string())
    };
    let display = if smin == smax { fmt(smin) } else { format!("{} - {}", fmt(smin), fmt(smax)) };
    (smin, smax, display)
}
...
struct TimingTags {
    bpms: String, stops: String, delays: String, warps: String,
    speeds: String, scrolls: String, fakes: String,
}
...
#[derive(Clone, Default)]
struct ChartTags {
    bpms: Option<String>, stops: Option<String>, delays: Option<String>, warps: Option<String>,
    speeds: Option<String>, scrolls: Option<String>, fakes: Option<String>,
}
...
impl ChartTags {
    ...
    fn resolve<'a>(&'a self, g: &'a TimingTags, use_chart: bool) -> [(&'a str, Option<&'a str>); 7] {
        macro_rules! pair { ($f:ident) => { (&g.$f, if use_chart { self.$f.as_deref() } else { None }) }; }
        [pair!(bpms), pair!(stops), pair!(delays), pair!(warps), pair!(speeds), pair!(scrolls), pair!(fakes)]
    }
}
...
fn chart_metadata(fields: &[&[u8]], fmt: TimingFormat) -> Option<(String, String)> {
    ...
    if step_type == "lights-cabinet" { return None; }
    ...
    Some((step_type, crate::resolve_difficulty_label(&diff_raw, &desc, &meter, ext)))
}

fn chart_bpm_snapshot(entry: &ParsedChartEntry<'_>, global: &TimingTags, bpms_norm: &str, 
                      fmt: TimingFormat, use_chart: bool) -> Option<ChartBpmSnapshot> {
    ...
    let hash_bpms = chart.bpms.as_ref()
    ...
    let segments = compute_timing_segments_cleaned(
        r[0].1, r[0].0, r[1].1, r[1].0, r[2].1, r[2].0, r[3].1, r[3].0,
        r[4].1, r[4].0, r[5].1, r[5].0, r[6].1, r[6].0, fmt,
    );

    let bpms: Vec<_> = segments.bpms.iter().map(|&(b, v)| (b as f64, v as f64)).collect();
    ...
    Some(ChartBpmSnapshot {
        step_type, difficulty, hash_bpms, bpms_formatted,
...
pub fn parse_bpm_map(s: &str) -> Vec<(f64, f64)> {
    let mut v: Vec<_> = s.split(',').filter_map(|c| {
        let c = c.trim();
...
pub fn get_current_bpm(beat: f64, map: &[(f64, f64)]) -> f64 {
    ...
    map[pos.saturating_sub(1).max(if pos == 0 { 0 } else { pos - 1 })].1
}

#[inline(always)]
pub(crate) fn for_each_measure_bpm<F: FnMut(usize, f64)>(
    count: usize, map: &[(f64, f64)], bpm: f64, mut f: F,
) {
    if count == 0 || map.is_empty() { return; }
...
fn bpm_range_filtered<T: PartialOrd + Default + Copy, F: Fn(f64) -> bool, M: Fn(f64) -> T>(
    map: &[(f64, f64)], filter: F, transform: M,
) -> (T, T) {
    if map.is_empty() { return (T::default(), T::default()); }
    let (mut min, mut max, mut count) = (f64::MAX, f64::MIN, 0);
    for &(_, bpm) in map {
        if filter(bpm) { min = min.min(bpm); max = max.max(bpm); count += 1; }
    }
    if count == 0 {
        for &(_, bpm) in map { min = min.min(bpm); max = max.max(bpm); }
    }
    (transform(min.max(0.0)), transform(max.max(0.0)))
}

pub fn get_elapsed_time(
    target: f64, bpms: &[(f64, f64)], stops: &[(f64, f64)], delays: &[(f64, f64)], warps: &[(f64, f64)],
) -> f64 {
...
fn elapsed_bpm_only(target: f64, bpms: &[(f64, f64)]) -> f64 {
    if bpms.is_empty() { return 0.0; }
    let (mut time, mut beat, mut bpm, mut i) = (0.0, 0.0, 60.0, 0);
    while i < bpms.len() && bpms[i].0 <= 0.0 { bpm = bpms[i].1; i += 1; }
    while i < bpms.len() && bpms[i].0 <= target {
        if bpms[i].0 > beat && bpm > 0.0 { time += (bpms[i].0 - beat) * 60.0 / bpm; }
        beat = bpms[i].0; bpm = bpms[i].1; i += 1;
    }
    if target > beat && bpm > 0.0 { time += (target - beat) * 60.0 / bpm; }
    time
}
...
fn elapsed_with_events(
    ...
    let mut events: Vec<_> = bpms.iter().map(|&(b, v)| (b, 0u8, v))
    ...
    let (mut time, mut beat, mut bpm, mut warp_end) = 
        (0.0, 0.0, if !bpms.is_empty() && bpms[0].0 <= 0.0 { bpms[0].1 } else { 60.0 }, 0.0);
    
    for (eb, pri, val) in events {
        if eb > target && warp_end <= target { break; }
        if eb > beat {
            let eff = beat.max(warp_end);
            if eb > eff && bpm > 0.0 { time += (eb - eff) * 60.0 / bpm; }
            beat = eb;
        }
        match pri { 0 => bpm = val, 1 => time += val, 2 => warp_end = warp_end.max(eb + val), _ => {} }
    }
    let eff = beat.max(warp_end);
    if target > eff && bpm > 0.0 { time += (target - eff) * 60.0 / bpm; }
    time
}
...
fn compute_last_beat_impl<const N: usize>(data: &[u8]) -> f64 {
    ...
    for line in data.split(|&b| b == b'\n') {
        ...
        match line[0] {
            b',' => { if last_m == Some(m) { rows = r; } m += 1; r = 0; continue; }
            b';' => { if last_m == Some(m) { rows = r; } break; }
            _ => {}
        }
        if line.len() < N { continue; }
        let mut has = false;
        for (c, &ch) in line[..N].iter().enumerate() {
            match ch {
                b'1' | b'M' | b'K' | b'L' | b'F' => has = true,
                b'2' | b'4' => holds[c] = holds[c].saturating_add(1),
                b'3' if holds[c] > 0 => { holds[c] -= 1; has = true; }
                _ => {}
            }
        }
        if has { last_m = Some(m); last_r = r; }
        r += 1;
    }
...
pub fn compute_last_beat(data: &[u8], lanes: usize) -> f64 {
    match lanes { 4 => compute_last_beat_impl::<4>(data), 8 => compute_last_beat_impl::<8>(data), _ => compute_last_beat_impl::<4>(data) }
}

pub fn compute_total_chart_length(
    data: &[u8], lanes: usize, bpms: &[(f64, f64)], stops: &[(f64, f64)], delays: &[(f64, f64)], warps: &[(f64, f64)],
) -> i32 {
    let beat = compute_last_beat(data, lanes);
    if beat <= 0.0 || bpms.is_empty() { 0 } else { get_elapsed_time(beat, bpms, stops, delays, warps).floor() as i32 }
}

pub fn compute_mines_nonfake(data: &[u8], lanes: usize, warps: &[(f64, f64)], fakes: &[(f64, f64)]) -> u32 {
    ...
    for line in data.split(|&b| b == b'\n') {
        if line.is_empty() { continue; }
        if line[0] == b',' { per_measure.push(cnt); m += 1; r = 0; cnt = 0; continue; }
        if line.len() < lanes { continue; }
        let mine = line[..lanes].iter().any(|&b| matches!(b, b'M' | b'm'));
        rows.push((m, r, mine)); cnt += 1; r += 1;
    }
    per_measure.push(cnt);

    let in_range = |b: f64, segs: &[(f64, f64)]| -> bool {
        let i = segs.partition_point(|(s, _)| *s <= b);
        i > 0 && { let (s, l) = segs[i-1]; l > 0.0 && l.is_finite() && b < s + l }
    };

    rows.iter().filter(|&&(m, r, mine)| {
...
pub fn compute_measure_nps_vec_with_timing(densities: &[usize], timing: &TimingData) -> Vec<f64> {
    ...
    densities.iter().map(|&d| {
        let (start, end) = (timing.get_time_for_beat_f32(beat), timing.get_time_for_beat_f32(beat + 4.0));
        beat += 4.0;
        let dur = end - start;
        if d == 0 || dur <= 0.12 { 0.0 } else { d as f64 / dur }
    }).collect()
}

fn compute_nps_iter<F: Fn(usize) -> f64>(densities: &[usize], get_bpm: F) -> Vec<f64> {
    densities.iter().enumerate().map(|(i, &d)| {
        let bpm = get_bpm(i);
        if d == 0 || !is_display_bpm(bpm) { 0.0 } else { d as f64 * bpm / 240.0 }
    }).collect()
}
...
fn median(arr: &[f64]) -> f64 {
    ...
    if v.len() % 2 == 1 { v[mid] } else { (v[..mid].iter().fold(f64::MIN, |a, &b| a.max(b)) + v[mid]) / 2.0 }
}

pub fn get_nps_stats(nps: &[f64]) -> (f64, f64) {
    (nps.iter().fold(f64::MIN, |a, &b| a.max(b)).max(0.0), median(nps))
}
...
pub fn compute_bpm_stats(values: &[f64]) -> (f64, f64) {
    ...
    if v.is_empty() { v.extend_from_slice(values); }
    v.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
    let med = if v.len() % 2 == 0 { (v[v.len()/2 - 1] + v[v.len()/2]) / 2.0 } else { v[v.len()/2] };
...
pub fn compute_tier_bpm(densities: &[usize], bpms: &[(f64, f64)], bpm: f64) -> f64 {
    ...
    let max_bpm = bpms.iter().map(|&(_, b)| b).filter(|&b| is_display_bpm(b))
    ...
    for_each_measure_bpm(densities.len(), bpms, bpm, |i, b| {
        ...
        if c == RunDensity::Break {
            if len >= 4 { max_e = max_e.max(run_e); }
            cat = RunDensity::Break; len = 0; run_e = 0.0;
        } else {
            if len == 0 || c != cat {
                if len >= 4 { max_e = max_e.max(run_e); }
                cat = c; len = 0; run_e = 0.0;
            }
            len += 1;
            if is_display_bpm(b) { run_e = run_e.max(densities[i] as f64 * b / 16.0); }
        }
    });
    if len >= 4 { max_e = max_e.max(run_e); }
...
pub fn normalize_and_tidy_bpms(param: &str) -> String {
    let mut entries: Vec<_> = param.split(',').enumerate().filter_map(|(i, e)| {
        let (b, v) = e.trim().split_once('=')?;
        let (bs, vs) = (normalize_decimal(b)?, normalize_decimal(v)?);
        Some((bs.parse::<i64>().ok()?, bs, vs.parse::<i64>().ok()?, vs, i))
    }).collect();
    
    if entries.is_empty() { return "0.000=60.000".to_string(); }
    entries.sort_by_key(|e| (e.0, e.4));
    
    // Dedupe by beat, keeping last
    let mut deduped = Vec::with_capacity(entries.len());
    for e in entries {
        if deduped.last().map_or(false, |l: &(i64, String, i64, String, usize)| l.0 == e.0) {
            *deduped.last_mut().unwrap() = e;
        } else { deduped.push(e); }
    }
    
    if let Some(f) = deduped.first_mut() { if f.0 != 0 { f.0 = 0; f.1 = "0.000".into(); } }
    
    // Remove consecutive same values
    let mut out = String::new();
    let mut last_v = None;
    for (_, bs, vt, vs, _) in deduped {
        if last_v == Some(vt) { continue; }
        last_v = Some(vt);
        if !out.is_empty() { out.push(','); }
        out.push_str(&bs); out.push('='); out.push_str(&vs);
    }
    out
}
