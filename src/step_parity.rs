...
impl Foot {
    ...
    #[inline(always)]
    fn is_left(self) -> bool { matches!(self, Foot::LeftHeel | Foot::LeftToe) }
    
    #[inline(always)]
    fn is_right(self) -> bool { matches!(self, Foot::RightHeel | Foot::RightToe) }
}

const NUM_FEET: usize = 5;
const MAX_COLUMNS: usize = 8;
const FEET: [Foot; 4] = [Foot::LeftHeel, Foot::LeftToe, Foot::RightHeel, Foot::RightToe];
...
const OTHER_PART_OF_FOOT: [Foot; NUM_FEET] = [
    Foot::None, Foot::LeftToe, Foot::LeftHeel, Foot::RightToe, Foot::RightHeel,
];
...
const LEFT_PAIR: FootPair = FootPair { heel: Foot::LeftHeel, toe: Foot::LeftToe };
const RIGHT_PAIR: FootPair = FootPair { heel: Foot::RightHeel, toe: Foot::RightToe };
...
impl NeighborMap {
    ...
    fn insert_reserved(&mut self, neighbor_id: usize, hash_key: usize, cost: f32) {
        ...
        if let Some(entry) = self.entries.iter_mut().find(|e| e.neighbor_id == neighbor_id) {
            entry.cost = cost;
            return;
        }
    ...
    fn insert_new_reserved(&mut self, neighbor_id: usize, hash_key: usize, cost: f32) {
        ...
        self.entries.push(NeighborEntry { neighbor_id, cost, next: BUCKET_EMPTY, hash_key });
        self.insert_index(idx);
    }

    fn get(&self, neighbor_id: usize) -> Option<f32> {
        self.entries.iter().find(|e| e.neighbor_id == neighbor_id).map(|e| e.cost)
    }
...
fn next_prime(start: usize) -> usize {
    ...
    while !is_prime(c) { c = c.saturating_add(2); }
    c
}
...
impl StageLayout {
    fn new_dance_single() -> Self {
        Self::new(
            vec![
                StagePoint { x: 0.0, y: 1.0 }, StagePoint { x: 1.0, y: 0.0 },
                StagePoint { x: 1.0, y: 2.0 }, StagePoint { x: 2.0, y: 1.0 },
            ],
            vec![2], vec![1], vec![0, 3],
        )
    }
    ...
    fn new_dance_double() -> Self {
        Self::new(
            vec![
                StagePoint { x: 0.0, y: 1.0 }, StagePoint { x: 1.0, y: 0.0 },
                StagePoint { x: 1.0, y: 2.0 }, StagePoint { x: 2.0, y: 1.0 },
                StagePoint { x: 3.0, y: 1.0 }, StagePoint { x: 4.0, y: 0.0 },
                StagePoint { x: 4.0, y: 2.0 }, StagePoint { x: 5.0, y: 1.0 },
            ],
            vec![2, 6], vec![1, 5], vec![0, 3, 4, 7],
        )
    }
    ...
    fn new(
        ...
        for left in 0..pair_stride {
            for right in 0..pair_stride {
                ...
                let lp = if left == invalid { None } else { Some(columns[left]) };
                let rp = if right == invalid { None } else { Some(columns[right]) };

                avg_points[idx] = match (lp, rp) {
                    (None, None) => StagePoint::default(),
                    (None, Some(r)) => r,
                    (Some(l), None) => l,
                    (Some(l), Some(r)) => StagePoint { x: (l.x + r.x) / 2.0, y: (l.y + r.y) / 2.0 },
                };

                if left == right || left == invalid || right == invalid { continue; }

                let (dx, dy) = ((columns[right].x - columns[left].x) as f64, 
        ...
        Self { columns, up_arrows, down_arrows, side_arrows, pair_stride, 
               avg_points, x_diffs, y_diffs, dist_stride, dist_sq, dist_weighted }
    }

    #[inline(always)]
    fn column_count(&self) -> usize { self.columns.len() }
    ...
    fn pair_index(&self, left: isize, right: isize) -> usize {
        ...
        let l = if left == INVALID_COLUMN { max } else { left as usize };
        let r = if right == INVALID_COLUMN { max } else { right as usize };
        l * self.pair_stride + r
    }

    #[inline(always)]
    fn get_x_diff(&self, l: isize, r: isize) -> f32 { self.x_diffs[self.pair_index(l, r)] }
    
    #[inline(always)]
    fn get_y_diff(&self, l: isize, r: isize) -> f32 { self.y_diffs[self.pair_index(l, r)] }
    
    #[inline(always)]
    fn avg_point(&self, l: isize, r: isize) -> StagePoint { self.avg_points[self.pair_index(l, r)] }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
enum TapNoteType { #[default] Empty, Tap, HoldHead, HoldTail, Mine, Fake }

#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
enum TapNoteSubType { #[default] Invalid, Hold, Roll }
...
impl Row {
    fn new(cols: usize) -> Self {
        Self {
            ...
            second: 0.0, beat: 0.0, row_index: 0, column_count: cols,
            note_count: 0, note_mask: 0, hold_mask: 0, 
            mine_mask: 0, mine_i32_mask: 0, fake_mine_mask: 0,
        }
    }
...
impl State {
    ...
    fn foot_moved_not_holding(&self, pair: &FootPair) -> bool {
        (self.did_the_foot_move[pair.heel.as_index()] && !self.is_the_foot_holding[pair.heel.as_index()])
        || (self.did_the_foot_move[pair.toe.as_index()] && !self.is_the_foot_holding[pair.toe.as_index()])
    }
}

impl PartialEq for State {
    fn eq(&self, other: &Self) -> bool {
        self.columns == other.columns && self.combined_columns == other.combined_columns
            && self.moved_feet == other.moved_feet && self.hold_feet == other.hold_feet
    }
}
impl Eq for State {}
...
impl NodeArena {
    ...
    fn clear(&mut self) {
        for chunk in &mut self.chunks { chunk.clear(); }
        self.len = 0;
    }

    fn push(&mut self, node: StepParityNode) -> usize {
        let idx = self.len;
        if self.chunks.last().map_or(true, |c| c.len() == NODE_CHUNK_SIZE) {
    ...
    fn get(&self, idx: usize) -> Option<&StepParityNode> {
        if idx < self.len {
            self.chunks.get(idx / NODE_CHUNK_SIZE)?.get(idx % NODE_CHUNK_SIZE)
        } else { None }
    }

    fn get_mut(&mut self, idx: usize) -> Option<&mut StepParityNode> {
        if idx < self.len {
            self.chunks.get_mut(idx / NODE_CHUNK_SIZE)?.get_mut(idx % NODE_CHUNK_SIZE)
        } else { None }
    }

    fn ptr(&self, idx: usize) -> *const StepParityNode {
        self.get(idx).map(|n| n as *const _).unwrap_or(std::ptr::null())
    }
}

impl Index<usize> for NodeArena {
    type Output = StepParityNode;
    fn index(&self, idx: usize) -> &Self::Output { self.get(idx).expect("node index out of bounds") }
}
...
fn did_jack(initial: &State, result: &State, pair: &FootPair,
            heel_col: isize, toe_col: isize, moved: bool, did_jump: bool) -> bool {
    if did_jump || !moved { return false; }
...
fn calc_action_cost(
    ...
    let did_jump = initial.foot_moved_not_holding(&LEFT_PAIR) 
                && initial.foot_moved_not_holding(&RIGHT_PAIR);
    ...
    cost += calc_bracket_jack_cost(result, moved_left, moved_right, jacked_left, jacked_right, did_jump);
    cost += calc_doublestep_cost(initial, result, rows, row_idx, moved_left, moved_right, jacked_left, jacked_right, did_jump);
...
fn calc_hold_switch_cost(layout: &StageLayout, initial: &State, result: &State, row: &Row) -> f32 {
    ...
    while mask != 0 {
        ...
        if switched {
            ...
            let dist = if prev_col == INVALID_COLUMN { 1.0 } 
                      else { (layout.get_distance_sq(c, prev_col as usize) as f64).sqrt() as f32 };
...
fn calc_bracket_tap_cost(initial: &State, row: &Row, lh: isize, lt: isize, rh: isize, rt: isize, elapsed: f32) -> f32 {
    if row.hold_mask == 0 { return 0.0; }
    let mut cost = 0.0;
    
    let check_pair = |heel: isize, toe: isize, pair: &FootPair| -> f32 {
        if heel == INVALID_COLUMN || toe == INVALID_COLUMN { return 0.0; }
        let jack_penalty = if initial.foot_moved(pair) { 1.0 / elapsed } else { 1.0 };
        let hm = (row.hold_mask & (1u8 << heel as usize)) != 0;
        let tm = (row.hold_mask & (1u8 << toe as usize)) != 0;
        if (hm && !tm) || (tm && !hm) { BRACKETTAP_WEIGHT * jack_penalty } else { 0.0 }
    };
...
fn calc_bracket_jack_cost(result: &State, moved_left: bool, moved_right: bool,
                          jacked_left: bool, jacked_right: bool, did_jump: bool) -> f32 {
    let hold_empty = result.hold_feet.iter().all(|&f| f == Foot::None);
    if moved_left == moved_right || !hold_empty || did_jump { return 0.0; }
...
fn calc_doublestep_cost(initial: &State, result: &State, rows: &[Row], row_idx: usize,
                        moved_left: bool, moved_right: bool, 
                        jacked_left: bool, jacked_right: bool, did_jump: bool) -> f32 {
    let hold_empty = result.hold_feet.iter().all(|&f| f == Foot::None);
    if moved_left == moved_right || !hold_empty || did_jump { return 0.0; }
    
    if did_double_step(initial, rows, row_idx, moved_left, jacked_left, moved_right, jacked_right) {
        DOUBLESTEP_WEIGHT
    } else { 0.0 }
}
...
fn calc_twisted_foot_cost(layout: &StageLayout, result: &State) -> f32 {
    ...
    let backward = |heel: isize, toe: isize| -> bool {
        heel != INVALID_COLUMN && toe != INVALID_COLUMN 
...
fn calc_facing_cost(layout: &StageLayout, result: &State) -> f32 {
    ...
    if lt == INVALID_COLUMN { lt = lh; }
    if rt == INVALID_COLUMN { rt = rh; }

    let facing = |a: isize, b: isize, f: fn(&StageLayout, isize, isize) -> f32| -> f32 {
        if a != INVALID_COLUMN && b != INVALID_COLUMN { f(layout, a, b) } else { 0.0 }
    };

    let penalty = |v: f32| -> f32 {
        let base = -(v.min(0.0));
        if base > 0.0 { (base as f64).powf(1.8) as f32 * 100.0 * FACING_WEIGHT } else { 0.0 }
    };
...
fn calc_spin_cost(layout: &StageLayout, initial: &State, result: &State) -> f32 {
    ...
    if lt == INVALID_COLUMN { lt = get(result, Foot::LeftHeel); }
    if rt == INVALID_COLUMN { rt = get(result, Foot::RightHeel); }
...
fn calc_footswitch_cost(initial: &State, result: &State, row: &Row, elapsed: f32, cols: usize) -> f32 {
    if elapsed < SLOW_FOOTSWITCH_THRESHOLD || elapsed >= SLOW_FOOTSWITCH_IGNORE { return 0.0; }
    if row.mine_i32_mask != 0 || row.fake_mine_mask != 0 { return 0.0; }

    let time_scaled = elapsed - SLOW_FOOTSWITCH_THRESHOLD;
    for i in 0..cols {
        let (init, res) = (initial.combined_columns[i], result.columns[i]);
        if init == Foot::None || res == Foot::None { continue; }
        if init != res && init != OTHER_PART_OF_FOOT[res.as_index()] {
            let divisor = SLOW_FOOTSWITCH_THRESHOLD + time_scaled;
            if divisor > 0.0 { return (time_scaled / divisor) * FOOTSWITCH_WEIGHT; }
        }
    }
    0.0
}
...
fn calc_sideswitch_cost(layout: &StageLayout, initial: &State, result: &State) -> f32 {
    layout.side_arrows.iter().filter(|&&c| {
...
fn calc_jack_cost(moved_left: bool, moved_right: bool, jacked_left: bool, jacked_right: bool, elapsed: f32) -> f32 {
    if elapsed < JACK_THRESHOLD && moved_left != moved_right && (jacked_left || jacked_right) {
        let ts = JACK_THRESHOLD - elapsed;
        if ts > 0.0 { return (1.0 / ts - 1.0 / JACK_THRESHOLD) * JACK_WEIGHT; }
    }
    0.0
}

fn calc_big_movements_cost(layout: &StageLayout, initial: &State, result: &State, elapsed: f32) -> f32 {
    let mut cost = 0.0;
    for &foot in &result.moved_feet {
        if foot == Foot::None { continue; }
        let init_pos = initial.where_the_feet_are[foot.as_index()];
        if init_pos == INVALID_COLUMN { continue; }
        ...
        if other_pos != INVALID_COLUMN {
            if other_pos == init_pos { continue; }
            d *= 0.2;
        }
        cost += d;
    }
...
fn did_double_step(initial: &State, rows: &[Row], row_idx: usize,
                   moved_left: bool, jacked_left: bool, 
                   moved_right: bool, jacked_right: bool) -> bool {
    let mut ds = false;
    if moved_left && !jacked_left && initial.foot_moved_not_holding(&LEFT_PAIR) { ds = true; }
    if moved_right && !jacked_right && initial.foot_moved_not_holding(&RIGHT_PAIR) { ds = true; }
...
impl StepParityGenerator {
    ...
    fn create_rows(&mut self, notes: Vec<IntermediateNoteData>) {
        ...
        for note in notes {
            if note.note_type == TapNoteType::Empty { continue; }

            if note.note_type == TapNoteType::Mine {
                let target = if note.second == counter.last_second && !self.rows.is_empty() {
                    if note.fake { &mut counter.next_fake_mines } else { &mut counter.next_mines }
                } else if note.fake { 
    ...
    fn flush_row(&mut self, counter: &mut RowCounter) {
        if counter.last_second == CLM_SECOND_INVALID { return; }
    ...
    fn build_row(&self, counter: &RowCounter) -> Row {
        ...
        for c in 0..self.column_count.min(MAX_COLUMNS) {
            ...
            if row.holds[c].note_type != TapNoteType::Empty { row.hold_mask |= 1u8 << c; }
            if row.mines[c] != 0.0 { row.mine_mask |= 1u8 << c; }
            if (row.mines[c] as i32) != 0 { row.mine_i32_mask |= 1u8 << c; }
            if (row.fake_mines[c] as i32) != 0 { row.fake_mine_mask |= 1u8 << c; }
        }
        row
    }
        ...
        for i in 0..self.rows.len() {
            ...
            for &init_id in &prev_ids {
                ...
                for perm in perms.iter() {
                    ...
                    let cost = calc_action_cost(&self.layout, &init_state, &result, &self.rows, i, elapsed);
    ...
    fn add_node(&mut self, state: Rc<State>, second: f32) -> usize {
        self.nodes.push(StepParityNode { state, second, neighbors: NeighborMap::default() })
    }
    ...
    fn perms_for_row(&mut self, row_idx: usize) -> Rc<[FootPlacement]> {
        ...
        if let Some(p) = self.permute_cache.get(&key) { return Rc::clone(p); }

        let mut cols = [Foot::None; MAX_COLUMNS];
        let mut perms = Vec::new();
        permute_row(&self.layout, row, &mut cols, 0, row.column_count, false, 0, &mut perms);
        if perms.is_empty() {
            permute_row(&self.layout, row, &mut cols, 0, row.column_count, true, 0, &mut perms);
        }
        if perms.is_empty() { perms.push([Foot::None; MAX_COLUMNS]); }
    ...
    fn init_result_state(&mut self, initial: &State, row_idx: usize, cols: &FootPlacement) -> Rc<State> {
        let row = &self.rows[row_idx];
        ...
        for i in 0..n {
            let foot = cols[i];
            hash = hash.wrapping_mul(STATE_HASH_PRIME).wrapping_add(foot as u64);
            if foot != Foot::None && ((hold_mask & (1u8 << i)) == 0 || initial.combined_columns[i] != foot) {
        ...
        for i in 0..n {
            let combined = if cols[i] != Foot::None { cols[i] }
            else {
                let prev = initial.combined_columns[i];
                match prev {
                    Foot::LeftHeel | Foot::RightHeel if (moved_mask & FOOT_MASKS[prev.as_index()]) == 0 => prev,
                    Foot::LeftToe if !moved_left => prev,
                    Foot::RightToe if !moved_right => prev,
                    _ => Foot::None,
                }
            };
            hash = hash.wrapping_mul(STATE_HASH_PRIME).wrapping_add(combined as u64);
        }
        
        // moved and hold hashes
        for i in 0..n {
            let foot = cols[i];
            let moved = if foot != Foot::None && ((hold_mask & (1u8 << i)) == 0 || initial.combined_columns[i] != foot) 
                { foot } else { Foot::None };
            hash = hash.wrapping_mul(STATE_HASH_PRIME).wrapping_add(moved as u64);
        }
        for i in 0..n {
            let hold = if cols[i] != Foot::None && (hold_mask & (1u8 << i)) != 0 { cols[i] } else { Foot::None };
            hash = hash.wrapping_mul(STATE_HASH_PRIME).wrapping_add(hold as u64);
        }

        if let Some(existing) = self.state_cache.get(&hash) { return Rc::clone(existing); }
    ...
    fn trace_path(&mut self) -> bool {
        ...
        if pred[n - 1] == usize::MAX { return false; }
        ...
        while cur != 0 {
            let prev = pred[cur];
            if prev == usize::MAX { return false; }
            cur = prev;
            if cur == 0 { break; }
...
fn permute_row(layout: &StageLayout, row: &Row, cols: &mut FootPlacement, 
               col: usize, col_count: usize, ignore_holds: bool, used: u8, out: &mut Vec<FootPlacement>) {
    if col >= col_count {
        let (mut lh, mut lt, mut rh, mut rt) = (INVALID_COLUMN, INVALID_COLUMN, INVALID_COLUMN, INVALID_COLUMN);
        ...
        || (rh == INVALID_COLUMN && rt != INVALID_COLUMN) { return; }
        
        // Bracket distance check
        if lh != INVALID_COLUMN && lt != INVALID_COLUMN && !layout.bracket_check(lh as usize, lt as usize) { return; }
        if rh != INVALID_COLUMN && rt != INVALID_COLUMN && !layout.bracket_check(rh as usize, rt as usize) { return; }
        
        out.push(*cols);
        return;
    }

    let mask = if ignore_holds { row.note_mask } else { row.note_mask | row.hold_mask };
    let active = (mask & (1u8 << col)) != 0;

    if active {
        for &foot in &FEET {
            let fm = FOOT_MASKS[foot.as_index()];
            if used & fm != 0 { continue; }
            cols[col] = foot;
            permute_row(layout, row, cols, col + 1, col_count, ignore_holds, used | fm, out);
            cols[col] = Foot::None;
        }
    } else {
        permute_row(layout, row, cols, col + 1, col_count, ignore_holds, used, out);
    }
}
...
fn calculate_tech_counts(rows: &[Row], layout: &StageLayout) -> TechCounts {
    ...
    for i in 1..rows.len() {
        ...
        if curr.note_count == 1 && prev.note_count == 1 {
            for &foot in &FEET {
                let (cc, pc) = (curr.where_the_feet_are[foot.as_index()], 
                               prev.where_the_feet_are[foot.as_index()]);
                if cc == INVALID_COLUMN || pc == INVALID_COLUMN { continue; }
                if cc == pc && elapsed < JACK_CUTOFF { out.jacks += 1; }
                else if cc != pc && elapsed < DOUBLESTEP_CUTOFF { out.doublesteps += 1; }
            }
        }

        // Brackets
        if curr.note_count >= 2 {
            if curr.where_the_feet_are[1] != INVALID_COLUMN && curr.where_the_feet_are[2] != INVALID_COLUMN {
                out.brackets += 1;
            }
            if curr.where_the_feet_are[3] != INVALID_COLUMN && curr.where_the_feet_are[4] != INVALID_COLUMN {
                out.brackets += 1;
            }
        }
        ...
        let is_switch = |c: usize| -> bool {
            ...
            p != Foot::None && r != Foot::None && p != r 
                && OTHER_PART_OF_FOOT[p.as_index()] != r && elapsed < FOOTSWITCH_CUTOFF
        };
        
        for &c in &layout.up_arrows { if is_switch(c) { out.up_footswitches += 1; out.footswitches += 1; } }
        for &c in &layout.down_arrows { if is_switch(c) { out.down_footswitches += 1; out.footswitches += 1; } }
        for &c in &layout.side_arrows { if is_switch(c) { out.sideswitches += 1; } }
...
fn trim_ws(mut s: &[u8]) -> &[u8] {
    while let Some((&f, r)) = s.split_first() { if f.is_ascii_whitespace() { s = r; } else { break; } }
    while let Some((&l, r)) = s.split_last() { if l.is_ascii_whitespace() { s = r; } else { break; } }
    s
}

#[inline(always)]
fn has_obj(line: &[u8]) -> bool { line.iter().any(|&b| b != b'0') }

fn parse_rows<F>(data: &[u8], cols: usize, mut get_second: F) -> Vec<ParsedRow>
where F: FnMut(f32) -> f32 
{
    let mut rows = Vec::new();
    if cols == 0 || cols > 8 { return rows; }

    let mut measure_idx = 0usize;
    for measure in data.split(|&b| b == b',') {
        let lines: Vec<_> = measure.split(|&b| b == b'\n')
            .map(trim_ws).filter(|l| !l.is_empty()).collect();
        if lines.is_empty() { measure_idx += 1; continue; }
        ...
        for (j, line) in lines.into_iter().enumerate() {
            ...
            rows.push(ParsedRow { chars, columns: cols as u8, row: note_row, beat, second });
        }
        measure_idx += 1;
    }
    rows
...
fn parse_rows_from_arrays<const LANES: usize>(
    ...
    if cols == 0 || cols > 8 { return parsed; }
    
    let copy_len = cols.min(LANES);
    for (idx, row) in rows.iter().enumerate() {
        if !has_obj(&row[..copy_len]) { continue; }
        ...
        parsed.push(ParsedRow { chars, columns: cols as u8, row: note_row, beat, second });
    }
    parsed
}
    ...
    for (idx, row) in rows.iter().enumerate() {
        for c in 0..cols {
            match row.chars[c] {
                ...
                b'3' => if let Some((si, sr)) = hold_starts[c] {
    ...
    for (idx, row) in rows.iter().enumerate() {
        ...
        for c in 0..cols {
            ...
            let mut note = IntermediateNoteData {
                note_type, col: c, row: row.row as usize, beat: row.beat,
                second: row.second, fake: note_type == TapNoteType::Fake || row_fake,
                subtype: match ch { b'4' => TapNoteSubType::Roll, b'2' => TapNoteSubType::Hold, _ => TapNoteSubType::Invalid },
                ..Default::default()
            };

            if note_type == TapNoteType::HoldHead {
                let len = lengths[idx * cols + c];
                if len >= MISSING_HOLD_LENGTH_BEATS { continue; }
...
pub fn analyze_lanes(data: &[u8], bpm_map: &[(f64, f64)], offset: f64, lanes: usize) -> TechCounts {
    let Some(layout) = layout_for_lanes(lanes) else { return TechCounts::default(); };
    ...
    if !generator.analyze(notes, layout.column_count()) { return TechCounts::default(); }
...
fn time_between_beats(start: f32, end: f32, bpm_map: &[(f64, f64)]) -> f64 {
    ...
    for &(beat, value) in bpm_map {
        if beat <= last { bpm = value; continue; }
        if beat >= end as f64 { break; }
