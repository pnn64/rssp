...
fn format_len<T>(value: Option<&[T]>) -> String {
    value.map_or_else(|| "-".to_string(), |v| v.len().to_string())
}
...
fn compare_step_artists(
    ...
    for ((step_type, difficulty), expected_indices) in harness_entries {
        let Some(actual_indices) = actual_map.get(&(step_type.clone(), difficulty.clone())) else {
            println!(
                "  step_artist {step_type} {difficulty}: baseline present, RSSP missing chart"
            );
            step_artist_ok = false;
            step_artist_errors.push(format!(
                "Step artist chart missing: {step_type} {difficulty}"
            ));
            continue;
        };
        ...
        for idx in 0..count {
            ...
            if status != "....ok" {
                step_artist_errors.push(format!(
                    "Step artist mismatch {step_type} {difficulty} [{desc_label}]: RSSP step_artist: {actual_val:?}, Golden step_artist: {expected_val:?}"
                ));
            }
        }
    }
    ...
    for ((step_type, difficulty), expected_indices) in harness_entries {
        ...
        for idx in 0..count {
            ...
            println!(
                ...
                actual_display_min.map_or_else(|| "-".to_string(), |v| v.to_string()),
                expected_display_max.map_or_else(|| "-".to_string(), |v| v.to_string()),
                actual_display_max.map_or_else(|| "-".to_string(), |v| v.to_string()),
                status
            );
        }
    ...
    for ((step_type, difficulty), expected_indices) in harness_entries {
        ...
        for idx in 0..count {
            ...
            println!(
                ...
                difficulty,
                meter_label,
                expected_val.map_or_else(|| "-".to_string(), |v| v.to_string()),
                actual_val.map_or_else(|| "-".to_string(), |v| v.to_string()),
                status
            );
        }
    ...
    for ((step_type, difficulty), expected_indices) in harness_entries {
        ...
        for idx in 0..count {
            ...
            println!(
                ...
                difficulty,
                meter_label,
                expected_peak.map_or_else(|| "-".to_string(), |v| v.to_string()),
                actual_peak.map_or_else(|| "-".to_string(), |v| v.to_string()),
...
fn compare_step_counts(
    ...
    for ((step_type, difficulty), expected_indices) in harness_entries {
        ...
        for idx in 0..count {
            ...
            println!(
                "  {step_type} {difficulty} [{meter_label}]: {holds} | {mines} | {rolls} | {notes} | {lifts} | {fakes} | {jumps} | {hands} | {total_steps} {status}"
            );
        }
...
fn compare_tech_counts(
    ...
    for ((step_type, difficulty), expected_indices) in harness_entries {
        ...
        for idx in 0..count {
            ...
            let expected_counts = expected.and_then(|entry| entry.tech_counts.as_ref());
            if expected.is_some() && expected_counts.is_none() {
                return Err(format!(
            ...
            let actual_counts = actual.and_then(|entry| entry.tech_counts.as_ref());

            let expected_crossovers = format_count(expected_counts.map(|c| c.crossovers));
            let actual_crossovers = format_count(actual_counts.map(|c| c.crossovers));
            let expected_footswitches = format_count(expected_counts.map(|c| c.footswitches));
            let actual_footswitches = format_count(actual_counts.map(|c| c.footswitches));
            let expected_sideswitches = format_count(expected_counts.map(|c| c.sideswitches));
            let actual_sideswitches = format_count(actual_counts.map(|c| c.sideswitches));
            let expected_jacks = format_count(expected_counts.map(|c| c.jacks));
            let actual_jacks = format_count(actual_counts.map(|c| c.jacks));
            let expected_brackets = format_count(expected_counts.map(|c| c.brackets));
            let actual_brackets = format_count(actual_counts.map(|c| c.brackets));
            let expected_doublesteps = format_count(expected_counts.map(|c| c.doublesteps));
            let actual_doublesteps = format_count(actual_counts.map(|c| c.doublesteps));

            let matches = expected_counts.is_some()
                && actual_counts.is_some()
                && expected_counts == actual_counts;

            let status = if matches { "....ok" } else { "....MISMATCH" };

            println!(
                "  {step_type} {difficulty} [{meter_label}]: crossovers {expected_crossovers}->{actual_crossovers} | footswitches {expected_footswitches}->{actual_footswitches} | sideswitches {expected_sideswitches}->{actual_sideswitches} | jacks {expected_jacks}->{actual_jacks} | brackets {expected_brackets}->{actual_brackets} | doublesteps {expected_doublesteps}->{actual_doublesteps} {status}"
            );
        }
        ...
        let matches = expected_indices.len() == actual_indices.len()
            && expected_indices
                ...
                .all(|(expected_idx, actual_idx)| {
                    let expected = &harness_charts[*expected_idx];
                    let actual = &actual_charts[*actual_idx];
                    let Some(ref expected_counts) = expected.tech_counts else {
                        return false;
                    };
                    let Some(ref actual_counts) = actual.tech_counts else {
                        return false;
                    };
                    expected_counts == actual_counts
                });
        if !matches {
            let expected_crossovers: Vec<u32> = expected_indices
                .iter()
                .filter_map(|&i| harness_charts[i].tech_counts.as_ref().map(|c| c.crossovers))
                .collect();
            let actual_crossovers: Vec<u32> = actual_indices
                .iter()
                .map(|&i| {
                    actual_charts[i]
                        .tech_counts
                        .as_ref()
                        .map_or(0, |c| c.crossovers)
                })
                .collect();
            ...
            let actual_sideswitches: Vec<u32> = actual_indices
                ...
                .map(|&i| {
                    actual_charts[i]
                        .tech_counts
                        .as_ref()
                        .map_or(0, |c| c.sideswitches)
                })
                .collect();
            let expected_jacks: Vec<u32> = expected_indices
                .iter()
                .filter_map(|&i| harness_charts[i].tech_counts.as_ref().map(|c| c.jacks))
                .collect();
            let actual_jacks: Vec<u32> = actual_indices
                .iter()
                .map(|&i| actual_charts[i].tech_counts.as_ref().map_or(0, |c| c.jacks))
                .collect();
            let expected_brackets: Vec<u32> = expected_indices
                .iter()
                .filter_map(|&i| harness_charts[i].tech_counts.as_ref().map(|c| c.brackets))
                .collect();
            let actual_brackets: Vec<u32> = actual_indices
                .iter()
                .map(|&i| {
                    actual_charts[i]
                        .tech_counts
                        .as_ref()
                        .map_or(0, |c| c.brackets)
                })
                .collect();
...
fn compare_rssp_unique(
    ...
    for ((step_type, difficulty), expected_indices) in rssp_entries {
        ...
        for idx in 0..count {
            ...
            println!(
                "  {step_type} {difficulty} [{meter_label}]: matrix_rating {expected_matrix} -> {actual_matrix} | detailed {expected_detail} -> {actual_detail} | partial {expected_partial} -> {actual_partial} | simple {expected_simple} -> {actual_simple} | candles {expected_candles} -> {actual_candles} | mono {expected_mono} -> {actual_mono} | boxes {expected_boxes} -> {actual_boxes} | anchors {expected_anchors} -> {actual_anchors} {status}"
            );
        }
    ...
    match env::var("RSSP_ALL_PARITY_BACKEND") {
        Ok(value) if value.eq_ignore_ascii_case("assp") => {
            let exe = env::var_os("ASSP_EXE")
                .map_or_else(|| default_assp_exe(manifest_dir), PathBuf::from);
...
fn main() {
    ...
    if !failures.is_empty() {
        ...
        for failure in &failures {
            ...
            println!();
            println!(
                "rerun: cargo test --test all_parity -- --exact {:?}",
                failure.name
            );
        }
        println!();
