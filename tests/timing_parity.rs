...
use rssp::{analyze, AnalysisOptions, ChartSummary, SimfileSummary};
use rssp::timing::{SpeedUnit, TimingData};
...
fn normalize_speeds(mut speeds: Vec<(f64, f64, f64, i32)>) -> Vec<(f64, f64, f64, i32)> {
    if speeds.is_empty() {
...
fn normalize_scrolls(mut scrolls: Vec<(f64, f64)>) -> Vec<(f64, f64)> {
    if scrolls.is_empty() {
...
fn build_timing_snapshot(chart: &ChartSummary, simfile: &SimfileSummary) -> TimingSnapshot {
    ...
    TimingSnapshot {
        ...
        bpms: timing.bpm_segments(),
        stops: timing.stops().iter().map(|seg| (seg.beat, seg.duration)).collect(),
        delays: timing.delays().iter().map(|seg| (seg.beat, seg.duration)).collect(),
        time_signatures,
        warps: timing.warps().iter().map(|seg| (seg.beat, seg.length)).collect(),
        labels,
        tickcounts,
        combos,
        speeds,
        scrolls,
        fakes: timing.fakes().iter().map(|seg| (seg.beat, seg.length)).collect(),
    }
}
